    ...
    <script>
        ...
        function next(get_collegeclassid, get_currentpage, get_pages) {
            if (get_currentpage == get_pages)
                return;
            get_currentpage++;
            location.href = "/studentDetails?method=findCollegeClassStudentByCollegeClassid&CollegeClassID=" + get_collegeclassid + "&page=" + get_currentpage;
        };
        ...
        function first(get_collegeclassid) {
            location.href = "/studentDetails?method=findCollegeClassStudentByCollegeClassid&CollegeClassID=" + get_collegeclassid + "&page=1";
        };
        function last(get_collegeclassid, get_pages) {
            location.href = "/studentDetails?method=findCollegeClassStudentByCollegeClassid&CollegeClassID=" + get_collegeclassid + "&page=" + get_pages;
        };
    </script>
</head>
...
<section>
    <div class="jumbotron">
        <div class="container">
            <nav class="navbar navbar-expand-lg navbar-light bg-light">
                <form class="form-inline my-2 my-lg-0" method="post" action="/studentDetails?method=findStudentInCollegeGrade">
                ...
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <button type="button" class="btn btn-info" rel="external nofollow" onclick="exportCollegeClassDetails(${collegeclassid})">
            ...
            <table class="table table-bordered table-hover text-center">
                ...
                <tbody>
                    <c:forEach items="${list}" var="collegeClassStudent">
                        <tr>
                            <td>${collegeClassStudent.studentid}</td>
                            <td><button id="${collegeClassStudent.studentid}" type="button" class="btn btn-info" onclick="openStudentDetails(this.id)">${collegeClassStudent.studentname}</button></td>
                            <td>${collegeClassStudent.collegeclassname}</td>
                            <td>${collegeClassStudent.coursenumber}</td>
                            <td>${collegeClassStudent.attendencetimes}</td>
                            <td>${collegeClassStudent.unattendencetimes}</td>
                        </tr>
                    </c:forEach>
                </tbody>
            </table>
