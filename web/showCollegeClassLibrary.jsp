...
<head>
    ...
    <script>
        ...
        function open_CollegeClassLibraryDetail(get_collegeclassid) {
            var url = "/studentDetails?method=findCollegeClassStudentLibraryByCollegeClassid&CollegeClassID=" + get_collegeclassid + "&page=1";
            window.location=url;
        };
        function exportCollegeGradeDetails() {
            var url1 ="/teacherExport?method=exportAllGradeForLibrary";
    ...
    <div class="jumbotron">
        <div class="container">
            ...
            <table class="table table-bordered table-hover text-center">
                ...
                <tbody>
                    <c:forEach items="${list}" var="collgeclassshowlibrary">
                        <tr>
                            <td>${collgeclassshowlibrary.grade}</td>
                            <td>${collgeclassshowlibrary.collegeclassname}</td>
                            <td>${collgeclassshowlibrary.studentnumber}</td>
                            <td>
                                <button type="button" id="${collgeclassshowlibrary.collegeclassid}" name="open_btn" class="btn btn-info"
                                        rel="external nofollow"
                                        onclick="open_CollegeClassLibraryDetail(this.id)">展开
                                </button>
                            </td>
                        </tr>
                    </c:forEach>
